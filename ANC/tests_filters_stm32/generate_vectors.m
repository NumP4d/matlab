...
sine_wave = @(f) sin(2*pi*f*time);
input = 0.6 * sine_wave(f3);

input(1:(60 + 10)*Fs) = zeros(size(input(1:(60 + 10)*Fs)));
% Normalization
%input = input ./ max(abs(input * 32768 / 32767));
...
expect = zeros(size(expect));

mkdir fxlms;
fid_in = fopen('fxlms/input_vector.csv', 'w');
fid_expect = fopen('fxlms/expect_vector.csv', 'w');
