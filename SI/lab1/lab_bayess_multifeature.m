...
% Test dla dobranego tego parametru
N = 10000;
...
% Naive Bayess classification method
cl_naive = NaiveBayess;
cl_naive = cl_naive.Train([A_learn; A_test], [d_learn; d_check]);
[d_naive, perror_naive] = cl_naive.Classify(A);
acc_naive = accuracy(d_naive, d)

cl_parzen = BayessParzen;
cl_parzen = cl_parzen.Train([A_learn; A_test], [d_learn; d_check], []);
[d_parzen, perror_parzen] = cl_parzen.Classify(A);
acc_parzen = accuracy(d_parzen, d)

cl_parzen = BayessParzen;
cl_parzen = cl_parzen.Train([A_learn; A_test], [d_learn; d_check], bw(imax));
[d_parzen, perror_parzen] = cl_parzen.Classify(A);
acc_parzen = accuracy(d_parzen, d)