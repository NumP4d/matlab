...
for i = 1:3
    ...
    for j = 1:N_monte_carlo
        ...
        [X_selected, i_features, variance] = selection_var(X_, N, 'n');
        
        k = size(X_selected, 1);
        itest = (rand(k, 1) < 0.4);
        ilearn = ~itest;
        
        X_learn = X_selected(ilearn, :);
        Dx = Dn{i};
        D_learn = Dx(ilearn);
        X_test = X_selected(itest, :);
        D_test = Dx(itest);
        ...
        [D_classifier, accuracy(j)] = classifier(X_learn, D_learn, X_test, D_test, num_trees);
